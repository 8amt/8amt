...
const PinFriends: React.FC = () => {
  ...
  const initialPinnedFriends = mockUsers.filter(
    (friend) => friend.pin && friend.friend
  );
  const [pinnedFriends, setPinnedFriends] =
    useState<Friend[]>(initialPinnedFriends);
  ...
  const handleSave = () => {
    ...
    setPinnedFriends(
      updatedFriends.filter((friend) => friend.pin && friend.friend)
    );
